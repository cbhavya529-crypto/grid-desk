...
from powerdesk.display import (
    CATEGORY_LABELS, breadth, date_label, day_label, escape_md, filter_news, filter_official,
    is_recent, md_link, official_kind_labels, quotes_frame, time_label,
)
from powerdesk.news import fetch_news
from powerdesk.official import fetch_official
...
@st.cache_data(ttl=config.OFFICIAL_TTL_SECONDS, show_spinner=False)
def load_official():
    return fetch_official(config.OFFICIAL_PAGES, config.OFFICIAL_FEEDS)
...
news_tab, official_tab, stocks_tab = st.tabs(["News", "Official", "Stocks"])
...
@st.fragment(run_every=config.OFFICIAL_TTL_SECONDS)
@guarded("Official")
def official_view():
    with st.spinner("Checking CEA, MNRE and PIB…"):
        items, errors = load_official()

    bodies = [p.body for p in config.OFFICIAL_PAGES] + [f.name for f in config.OFFICIAL_FEEDS]
    kind_labels = official_kind_labels()
    picked_bodies = st.pills("Source", bodies, selection_mode="multi", key="o_bodies") or []
    picked_kinds = st.pills(
        "Type", list(kind_labels), selection_mode="multi", key="o_kinds",
        format_func=lambda k: kind_labels.get(k, k),
    ) or []
    query = st.text_input("Search notices", placeholder="Regulation, ALMM, BESS, capacity…", key="o_q")

    shown = filter_official(items, picked_bodies, picked_kinds, query)
    st.caption(f"{len(shown)} of {len(items)} notices from the last {config.OFFICIAL_MAX_AGE_DAYS} days. "
               f"Checked every {config.OFFICIAL_TTL_SECONDS // 3600} hours. 🆕 = last 7 days.")

    if not items:
        st.warning("No official notices loaded. The government sites may be slow or down; press “Refresh now” later.")
    elif not shown:
        st.info("No notices match these filters. Clear the search or pick other sources.")

    for day, group in groupby(shown[:250], key=lambda a: date_label(a.published)):
        st.subheader(day, divider="gray")
        for a in group:
            new = "🆕 " if is_recent(a.published) else ""
            st.markdown(f"{new}**{md_link(a.title, a.link)}**")
            st.caption(escape_md(f"{a.body} · {kind_labels.get(a.kind, a.kind)}"))
